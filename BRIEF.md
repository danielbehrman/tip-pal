...
3. **Sept 2 showing no History icon is by design, not a bug** — verified with the exact numbers: `cycle_start_date` is Sept 1, `floor_week/floor_day` is week 1 day 3, so days before the floor (Sept 1–2, i.e. "day 1" and "day 2" of the new cycle) are intentionally excluded from backfill, per round 3's documented conclusion. Sept 1's red X is a leftover Visit-10 entry from the *old* cycle, not a new-cycle gap.

Fixed (commit `b823b9b`): `DayEditor` now shows an inline hint on every locked treatment checkbox — `"Edit Week X · Day Y to change this"` (or the ramp/not-tracked reason) — computed from the same edge-state check that locks it, so a family can find the one editable day instead of guessing; the Today-tab banner now links to History instead of the dead arrow instruction. No changes to the position/backfill logic itself — round 3's fix already had the right data in place, families just had no way to see what it meant. Ready to merge: Yes, pending Project Owner verification below.
...
Next Action (immediate, on resume): Open **History → Sept 3** (not Sept 2, 4, 5, or 6 — those are correctly locked and will now say so), tap **Edit**, check **Peanut** (Walnut is already checked there), tap **Save**. This is the one day/food combination the app will currently accept. After saving, reload the Today tab and confirm: the back arrow is no longer permanently disabled, and the header position has moved past week 1 day 3. Report back exactly what you see (screenshot + which day) if anything still looks wrong.
