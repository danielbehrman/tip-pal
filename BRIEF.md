...
## Current Status
Phase: Phase 4 — Engagement — Reaction Ramp complete (Project Owner UI sign-off received 2026-09-01). Travel Day Buffer complete (Project Owner UI sign-off pending). Dogfooding bug found 2026-09-03 (Today tab back-arrow doesn't advance position) triggered a proper redesign, now mid-build: History Calendar + Unified Day Editing, 6 of 8 tasks complete as of 2026-09-05. Remaining Phase 4 backlog (Milestone Email, Emergency Medication Expiry Tracker, Native Push Notifications) is on hold until this dogfooding fix and the Travel Day Buffer sign-off both close.
Mode: Active Build
Last Updated: 2026-09-05
Blocker: None. History Calendar + Unified Day Editing (`plans/DOGFOODING-HISTORY-CALENDAR.md`) — Tasks 1-6 of 8 complete, each independently reviewed clean (Task 4, the shared `DayEditor` component, went through 4 review rounds at the highest scrutiny used this project given it controls all future position corrections — final state has zero open findings). Two genuine bugs found and fixed in the plan's own reference code during implementation, not implementer error: Task 5's date-boundary formula and Task 6's UTC-vs-local "today" determination — both independently re-verified correct by a second reviewer with fresh scenarios. Remaining: Task 7 (rewire `app/history/page.tsx` to the calendar), Task 8 (remove the superseded `/history/edit` surface), then Project Owner UI sign-off on both this and Travel Day Buffer together. Full task ledger: `.superpowers/sdd/progress.md`.
...
Next Action: Finish Tasks 7-8 of the History Calendar plan (History page rewire, remove `/history/edit`), then Project Owner UI sign-off on both Travel Day Buffer (onboarding Yes/No question, Settings toggle, daily view buffer number + info-sheet copy) and the History Calendar (Today-tab 10-day shortcut, calendar day-status icons, view/edit/save/confirm flow) — required regardless of review status per this project's UI gate rule. After both sign-offs, select and ticket the next Phase 4 feature (Milestone Email or Emergency Medication Expiry Tracker) via PM Agent.
...
| `dose_log.completed_at` timestamps are written inconsistently: most call sites use `new Date().toISOString()` (UTC-anchored), while the lazy auto-rollover reconciliation deliberately anchors to `T12:00:00.000Z` for a specific idempotency reason. Any component that derives a calendar-day key by slicing `completedAt` (e.g. `components/HistoryCalendar.tsx`'s `byDate` map) inherits a UTC-vs-local mismatch for users outside UTC — an entry logged near local midnight can bucket under the wrong day. Found and fixed the same class of bug in `HistoryCalendar`'s own `todayStr`/`isToday`/`isFuture` logic (now local-basis via `todayDateString()`), but the `completedAt`-keyed lookup itself was correctly judged out of scope for a single-component patch — it needs one project-wide decision on local- vs UTC-anchored `dose_log` timestamps, not a per-consumer workaround. | Medium | Found during the History Calendar build, 2026-09-05 | Open — not in scope for the current bundle. Candidate for the next Stable/maintenance pass, likely bundled with the existing `(week, day)`-keyed `dose_log` lookup carry-forward item above, since both stem from `dose_log` rows lacking a consistently-anchored date/epoch key. |
