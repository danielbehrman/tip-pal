...
Next Action: Project Owner UI sign-off on Travel Day Buffer (onboarding required Yes/No question, Settings toggle, daily view buffer number + info-sheet copy) — required regardless of review status per this project's UI gate rule. Brainstorm the Today-tab/History redesign (full-current-week back-nav, unify the two edit data sources, collapse-by-visit in History) before building it. After both, select and ticket the next Phase 4 feature (Milestone Email or Emergency Medication Expiry Tracker) via PM Agent.
...
| Today tab's back-arrow (`DailyView.handleNavigate`/`handleCheck`) edits a local scratch blob (`doseState.completedDays`) that never advances position or touches `dose_log` — the working correction flow lives only at `/history/edit`, which the Daily page never linked to. Discoverability half fixed (banner now links to `/history/edit`, commit `ff653fb`); the underlying data-source split (local blob vs. real `dose_log` row) is still unresolved. | High | Dogfooding, 2026-09-03 | Partially resolved (banner link). Open: unify the two data sources and extend back-nav to the full current week — Project Owner explicitly chose to brainstorm this properly rather than rush a fix. First-priority for next session, alongside collapsing past visits in History (new request, same session). |
