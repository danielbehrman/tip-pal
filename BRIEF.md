...
Next Action (immediate): final whole-branch review of the Trailing Edit Redesign worktree (in progress), then hand off to Project Owner for review, UI sign-off, and merge.
...
**Not yet done:** merge to `main`, Project Owner UI sign-off (per the standing gate rule — new/manual edit screen, `/re-parse`, and the Trailing Edit flow are all UI-facing regardless of review status), and applying the deferred migration at actual deploy time.
