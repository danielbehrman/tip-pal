...
Next Action: "Treatment Food Tracking Fixes" bundle (`plans/PHASE-STABLE-F1.md`, all 9 tasks) is code-complete and Reviewer-passed — `npx tsc --noEmit` and `npm run build` both fully clean repo-wide. Final whole-branch review is next, then this needs Dan's UI sign-off (confirm dialog copy/layout, banner copy, Settings per-food screen — all three required per standard gate) before it can be marked done in Carry Forward. Local QA was code-trace only, not live-interactive — local `.env.local` has empty Supabase credentials (pre-existing gap, same as Phase 3.6's Task 18). Phase 3 F1 Capacitor Simulator auth-flow re-verify still outstanding, blocking Phase 3 F7/F8 whenever that track resumes.
...
| False "yesterday wasn't completed" banner shows after manual day/week reset in Settings, even when today's foods are all checked | High | Dogfooding, 2026-07-14 | ✅ Implemented 2026-07-16 (Treatment Food Tracking Fixes bundle) — see `docs/superpowers/specs/2026-07-16-treatment-food-tracking-fixes-final-design.md` and `plans/PHASE-STABLE-F1.md`. Root cause fixed (floor-position comparison before running the yesterday check), plus two-variant single/multi-day gap copy. Pending QA + Dan UI sign-off (banner copy — required per standard gate). |
| Settings week/day adjuster is global-only — doesn't support per-food position, but treatment foods have tracked independently via `treatment_food_progress` since Phase 3.5 | Medium | Dogfooding, 2026-07-14 | ✅ Implemented 2026-07-16 (Treatment Food Tracking Fixes bundle) — single global stepper and bulk catch-up log fully removed (Settings and onboarding), replaced with one Week/Day stepper pair per treatment food and a read-only auto-derived "Program day" summary. Pending QA + Dan UI sign-off (Settings per-food screen — required per standard gate). |
| Complete Day was gated on 100% of treatment foods checked (should be per-food independent); day got stuck after a partial completion, blocking back-nav | High | Dogfooding, 2026-07-14 | ✅ Implemented 2026-07-16 (Treatment Food Tracking Fixes bundle) — gate removed entirely, replaced with a confirm-on-save dialog (names unchecked foods) and a lazy auto-rollover mechanism that finalizes only the single most recent missed day. Also amends Day navigation and Trailing Edit (Trailing Edit now advances a food's position on correction) — see Architecture Decisions above. Pending QA + Dan UI sign-off (confirm dialog copy/layout — required per standard gate). |
