...
Next Action: Final Architect design locked with Dan for the "Treatment Food Tracking Fixes" bundle — see `docs/superpowers/specs/2026-07-16-treatment-food-tracking-fixes-final-design.md`. Phase 2 Architecture Decisions table amended (Day completion rule, Day navigation, Trailing edit rows; Skip Session evening-skip path removed from active spec). Bulk catch-up log (`saveBulkCatchUpLog`) and its Settings modal are being removed entirely, not fixed — history is factual/never fabricated. Next: run `/write-plan` to produce `plans/PHASE-STABLE-F1.md`, then Dev. Phase 3 F1 Capacitor Simulator auth-flow re-verify still outstanding, blocking Phase 3 F7/F8 whenever that track resumes.
...
| False "yesterday wasn't completed" banner shows after manual day/week reset in Settings, even when today's foods are all checked | High | Dogfooding, 2026-07-14 | ⚠️ Rolled into the Treatment Food Tracking Fixes bundle (2026-07-15) — see `docs/superpowers/specs/2026-07-16-treatment-food-tracking-fixes-final-design.md`. Root cause confirmed (cycleStartDate backdating), fix approach locked, plan pending. |
| Settings week/day adjuster is global-only — doesn't support per-food position, but treatment foods have tracked independently via `treatment_food_progress` since Phase 3.5 | Medium | Dogfooding, 2026-07-14 | ⚠️ Rolled into the Treatment Food Tracking Fixes bundle (2026-07-15) — see `docs/superpowers/specs/2026-07-16-treatment-food-tracking-fixes-final-design.md`. Design confirmed via mockup with Dan, plan pending. |
