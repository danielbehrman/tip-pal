...
- **Reparse:** after `handleConfirm`'s existing writes succeed, a new step prompts for each treatment food in the *new* schedule (both carried-over and newly-introduced foods) via the same `FoodPositionStepper`, defaulting to Week 1/Day 1. On confirm, this new step calls `replaceFoodProgress` (see "Orphaned rows" below) — this replaces `archiveAndStartNewCycle`'s current silent no-op on `treatment_food_progress`.
...
### Orphaned `treatment_food_progress` rows on reparse (resolved)

**Problem:** `seedFoodProgress`'s per-food upsert (`onConflict: "family_id,food_name"`) only writes rows for foods present in the new schedule. A food dropped between cycles (already a real, expected case — `new-cycle/page.tsx`'s review screen has a "Removed" diff badge for exactly this) leaves its old `treatment_food_progress` row untouched. `fetchFoodProgress` (`lib/supabase.ts:604-622`) reads every row for the family with no filter by active schedule, and all three current callers of `getGlobalPosition` (`daily/page.tsx`, `history/edit/page.tsx`, `settings/page.tsx`) pass that unfiltered map straight through — so an orphaned row silently skews position math, the same failure mode item 3 is otherwise fixing, reintroduced from the removed-food direction.

**Resolution: (a) — delete stale rows, don't filter the read path.** `getGlobalPosition`/`getFurthestAheadPosition` (section 4) stay naive iterators with no schedule-awareness added — the invariant they rely on is "the table only ever contains rows for currently-active treatment foods," enforced at write time, not read time. Filtering at every read call site (`daily`, `history/edit`, `settings`, plus the new `foods/page.tsx` fetch in section 4) would touch code well outside this bundle's stated scope for no benefit over a single write-side fix.

Since section 3 already prompts for and seeds **every** food in the new schedule (not a diff), a full replace has no merge case to get wrong: add `replaceFoodProgress(entries: { foodName: string; week: number; day: number }[]): Promise<void>` to `lib/supabase.ts` — deletes all `treatment_food_progress` rows for the family, then inserts the given entries. This is also the semantically correct reparse behavior independent of the orphan problem: the existing review screen already tells the user treatment positions "Reset to Week 1, Day 1" on reparse, i.e. carried-over foods are not meant to retain their old `completedDays` either — full replace, not merge, is what's already promised. `seedFoodProgress` itself is unchanged and keeps its onboarding-only per-food-upsert behavior (onboarding always starts from an empty table, so upsert-only is already correct there and doesn't need the delete).

### Partial-write failure on the reparse position-save step (resolved)

**Write ordering:** `archiveAndStartNewCycle` (families → schedules → dose_state) is unchanged by this bundle — still 3 writes, still non-transactional, same as every other multi-write flow in this codebase (no RPC/transaction usage exists anywhere in `lib/supabase.ts` today; this is a pre-existing, out-of-scope characteristic, not something introduced here). Per the design above, `replaceFoodProgress` is called separately, from the new position-prompt screen's own confirm action — **after** `handleConfirm`'s 3 writes have already completed and the view has advanced past them. It is not chained inside `archiveAndStartNewCycle`, so it adds no new failure surface to that existing function.

**If `replaceFoodProgress` fails:** the position-prompt screen shows an inline error and stays on that screen — same pattern as onboarding's existing `saveError` state (`app/onboarding/page.tsx:145-147`: catch, set error message, stop `saving`, do not navigate). The user retries by re-tapping confirm; nothing upstream (families/schedules/dose_state) needs to be redone since those writes already succeeded and are not re-sent.

**If the user abandons without retrying** (closes the app, navigates away): `treatment_food_progress` is left empty for that family — either because the delete half of `replaceFoodProgress` succeeded but the insert half didn't, or because the call never ran. This is not a stuck or broken state: `daily/page.tsx:107-119`'s existing lazy-seed fallback already handles `progress.size === 0` by calling `seedFoodProgress` uniformly against `dose_state`'s week/day (which is 1/1, since `archiveAndStartNewCycle`'s write 3 already reset it) on the next daily-view load. `getGlobalPosition` also already treats an empty map as `{week: 1, day: 1}` (`lib/schedule.ts:109`), so no intermediate read path breaks on the empty-table window. Net effect of abandonment: the user's per-food position customization from the prompt screen is silently lost and every food falls back to the safe uniform Week 1/Day 1 default — not data corruption, not a crash, just a degrade to the pre-this-bundle default behavior. No new rollback/transaction machinery is being built to prevent this; the existing fallback already covers it.
